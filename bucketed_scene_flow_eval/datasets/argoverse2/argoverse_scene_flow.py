...
class ArgoverseSceneMultiStepFlowSequence(ArgoverseSceneFlowSequence):
    def __init__(
        self,
        log_id: str,
        dataset_dir: Path,
    ...
    def _load_no_flow(
        ...
        classes_0 = self._make_default_classes(raw_item.pc.pc)
        flow = MultiStepEgoLidarFlow.make_no_flow(len(classes_0))
        return self._make_tssf_item(raw_item, classes_0, flow), metadata
    ...
    def _load_with_flow(
        ...
        flow = MultiStepEgoLidarFlow(full_flow=ego_flow_with_ground[0], mask=is_valid_flow_with_ground_arr, multi_step_flows=ego_flow_with_ground[1:])
        return (self._make_tssf_item(raw_item, classes_0_with_ground, flow), metadata)


class ArgoverseMultiStepFlowSequenceLoader(ArgoverseSceneFlowSequenceLoader):
    def __init__(
        self,
        raw_data_path: Union[Path, list[Path]], 
        flow_data_path: Optional[Union[Path, list[Path]]] = None,
    ...
    def _sanitize_flow_data_path(
        ...
        if not flow_data_path is None:
            return self._sanitize_raw_data_path(flow_data_path)
        elif not use_gt_flow and flow_data_path is None:
            raise ValueError("flow_data_path must be provided if use_gt_flow is False")
        else:
            flow_suffix = "_sceneflow_feather"
            flow_paths = [path.parent / (path.name + flow_suffix) for path in raw_data_path]
            return flow_paths
        
    def load_sequence(self, sequence_id: str) -> ArgoverseSceneMultiStepFlowSequence:
        return super().load_sequence(sequence_id)
    ...
    def _load_sequence_uncached(self, sequence_id: str) -> ArgoverseSceneMultiStepFlowSequence:
        assert (
            sequence_id in self.sequence_id_to_flow_data
        ), f"sequence_id {sequence_id} does not exist"
        return ArgoverseSceneMultiStepFlowSequence(
            sequence_id,
            self.sequence_id_to_raw_data[sequence_id],
            self.sequence_id_to_flow_data[sequence_id],
            with_classes=self.use_gt_flow,
    ...
    def cache_folder_name(self) -> str:
        return f"av2_raw_data_use_gt_flow_{self.use_gt_flow}_raw_data_path_{self.raw_data_path}_flow_data_path_{self.flow_data_path}_rollout_steps_{self.rollout_steps}"